...
class BERTBILSTMCRF(object):
    ...
    def creat_model(self):
        ...
        model = keras_bert.load_trained_model_from_checkpoint(self.config_path,                                               checkpoint_file=self.check_point_path,
seq_len=self.max_len,
trainable=True)

        print('We just load the bert Model successfully!')
        inputs = model.inputs
...
if __name__ == '__main__':
    ...
    dp = DataProcess(data_type='mydata', model='bert')
    ...
    model.fit(train_data, train_label, batch_size=8, epochs=2,
    ...
    # num2tag = dp.num2tag()  # number to tag (the label)
    # i2w = dp.i2w()   # index to word
    # texts = []
    # texts.append(f"字符\t预测tag\t真实tag\n") 
    ...
    # f1score = f1_score(label_indexs, pridict_indexs, average='macro')  # 取宏平均
